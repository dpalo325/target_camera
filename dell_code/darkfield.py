...
##-- starts the process to capture an image
	...
	if Tis.Snap_image(1) is True:  # Snap an image with one second timeout
		...
		filename = 'UCID'+time+'.jpg'
		print("", filename,":",sep="")
		cv2.imwrite('images/UCID'+time+'.jpg',image) # Saves the captured image
		check = True
                
	else:
		print('Image not captured') # Reads out if image is not captured
...
##-- initialize the SSH connection
	...
	# sftp.put('compressed.jpeg', '/home/meg/meg2/online/web/mhttpd/LatestTargetImage.bmp', confirm=True)
