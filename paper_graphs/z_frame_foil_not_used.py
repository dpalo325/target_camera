...
gROOT.ForceStyle()
#tree.ReadFile("/home/dylan/Downloads/frame_foil429.csv","time:dx:dz:x:z", ',');
tree.ReadFile("/home/dylan/Downloads/t_dx_x.csv","time:dx:x", ',');
...
for i in range(N):
    tree.GetEntry(i)
    xn, timen, yn = tree.x*1000, tree.time, tree.dx
...
gr1.Draw("ap1")
gr1.GetYaxis().SetTitle("#Delta X^{T} [#mu m]")
...
#line2.Draw()
gr2.GetYaxis().SetTitle("X^{T}_{Frame} - X^{T}_{Foil} [mm]")
